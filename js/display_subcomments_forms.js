...
function openForm(event) {
  ...
  if (event.target && (event.target.classList.contains('reply_btn') || event.target.parentElement.classList.contains('reply_btn'))) {
    ...
    if (!replyButton.classList.contains('toggled') && !replyButton.classList.contains('multisubcom')) {
      ...
      const commentContainer = event.target.closest('.project_comment_itself');
      const commentId = Number(commentContainer.getAttribute('data-cmid'));
      const newForm = document.createElement('form');
      newForm.classList.add('project_comment_post','subcomment', 'first-subcomment');
      const projectId = document.getElementById('project-id').value;
      const newFormChunk1 = '<textarea name="comment_input" id="comment_input" class="comment_input" placeholder="Ajoutez votre réponse"></textarea><input type="hidden" name="pj_id" value="';
      ...
      const newForm = document.createElement('form');
      newForm.classList.add('project_comment_post','subcomment');
      const projectId = document.getElementById('project-id').value;
      const newFormChunk1 = '<textarea name="comment_input" id="comment_input" class="comment_input" placeholder="Ajoutez votre réponse"></textarea><input type="hidden" name="pj_id" value="';
      const newFormChunk2 = '"><input type="hidden" name="cm_id" value="';
      const newFormChunk3 = '"><button class="nice-blue-button" id="comment_input_btn"><img class="dblock" src="../img/icons/roundtick_yel.svg"><p>Publier</p></button>';
